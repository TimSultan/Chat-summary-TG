...
async def fetch_range_messages_cached(
    ...
    each day's raw transcript to disk (transcript_cache.py). A day already cached is
    reused instead of re-fetched -- permanently for closed-out past days, or within the
    last `transcript_cache.TODAY_TTL_SECONDS` for today. Different questions about the
    same day therefore share one Telegram fetch; only the (always-fresh) LLM call
    varies per question. `force_refresh=True` bypasses the cache and re-fetches every
    day regardless of freshness."""
    if start_day > end_day:
        raise ChatSummaryError(f"start_day ({start_day}) is after end_day ({end_day}).")
    ...
    while day <= end_day:
        ...
        if cached is not None:
            ...
            all_messages.extend(_message_from_dict(d) for d in cached)
        else:
