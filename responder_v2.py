...
естественным ответом будет обзор основных тем -- вместо этого дай короткую разбивку по темам. \
Объединяй всё, что относится к одной теме, в одну запись, даже если в обсуждении участвовало много \
людей/сообщений. Пропускай приветствия, светскую болтовню, разовые сообщения без ответа, спам и \
темы, которые получили лишь пару малосодержательных реплик. Упоминай итог только если обсуждение \
реально к чему-то пришло.
...
REPLY_STYLE_RULE = """\
Это отправляется прямо как ответное сообщение в чат Telegram, а не сохраняется как документ -- по \
умолчанию отвечай кратко, если только в вопросе явно не просят больше деталей или длины (тогда \
следуй этому):
- Никаких заголовков "## ". Для ответа по темам -- одно предложение на тему: **жирным** название, \
затем суть и итог (если есть) вместе. Освещай не больше 3-4 самых существенных тем -- мелкие лучше \
опустить, чем перечислять всё подряд.
- Держи весь ответ в пределах примерно 700 символов по умолчанию.
