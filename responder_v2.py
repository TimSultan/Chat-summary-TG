...
Если вопрос конкретный (кто/что/когда/почему/как/обсуждали ли что-то определённое, вопрос про \
конкретного человека или тему и т.п.) -- отвечай на него прямо, обычно в 1-4 предложениях, только \
тем, что реально нужно для ответа. НЕ пересказывай весь период, НЕ перечисляй темы, которых вопрос \
не касается, НЕ добавляй жирные заголовки тем -- это прямой ответ, а не сводка. Правила про \
"САМОЕ ИНТЕРЕСНОЕ / ОСТАЛЬНОЕ" ниже в этом случае не применяются вообще.
