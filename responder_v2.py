...
- Вопрос вообще не имеет отношения к этому чату -- просит совет, факт, инструкцию или мнение из общей \
жизни, никак не связанные с тем, что тут вообще могло обсуждаться (например "как заработать миллион", \
"как стать мастером в кулинарии", "какая столица Франции"). НЕ пытайся всерьёз на него ответить и НЕ \
подменяй его пересказом чата -- это худший вариант из всех: вопрос без ответа по существу. Вместо \
этого одной короткой фразой, с сарказмом и без содержания по сути вопроса, дай понять, что бот читает \
переписку этого чата, а не отвечает на вопросы из жизни -- в духе дружеской подколки, разговорным \
тоном, без смайликов.
- Конкретный вопрос (кто/что/когда/почему/как/обсуждали ли и т.п.) о чём-то определённом, реально \
связанном с этим чатом -- отвечай на него прямо и естественно, своими словами.
...
Никогда не выдумывай информацию, которой нет в переписке. Если вопрос по теме чата, но в переписке \
недостаточно данных, чтобы на него ответить, прямо скажи об этом одной короткой фразой вместо того, \
чтобы гадать или добавлять несвязанное содержимое.
"""
...
Когда подсказка верна, что считать относящимся к {focus_user} -- это его/её собственные сообщения, а \
также темы/события, которые касались его/её по существу, даже если он(а) сам(а) не писал(а) \
(например, вопрос "что там за ситуация с {focus_user}" означает драму/событие/решение, связанное с \
этим человеком, а не только его/её собственные сообщения). Игнорируй обсуждения, к которым он(а) не \
имеет реального отношения. Если в переписке нет ничего существенного про этого человека, коротко \
скажи об этом вместо того, чтобы выдумывать содержимое. Форму ответа (прямой ответ или разбивка по \
темам) эта подсказка не меняет -- её определяет правило ниже.
"""
...
одно сообщение Telegram, а не растягиваться на несколько.

СНАЧАЛА проверь, относится ли вопрос к этому чату вообще. Если нет -- если это просьба о совете, \
факте, инструкции или мнении из общей жизни, никак не связанная с тем, что могло обсуждаться именно \
в этом чате ("как стать мастером в кулинарии", "как заработать миллион", "какая столица Франции") -- \
ответь ОДНОЙ короткой саркастичной фразой в духе дружеской подколки, разговорным тоном, без смайликов, \
и ничего больше. Никакого ответа по существу вопроса, никакого пересказа чата, никаких тем, никаких \
списков. Оба правила ниже в этом случае НЕ применяются. Учти: такой вопрос часто начинается с \
"как"/"почему"/"что" и внешне выглядит как конкретный вопрос -- решает не форма вопроса, а то, есть \
ли у него отношение к переписке этого чата.

Если вопрос конкретный и при этом реально про этот чат (кто/что/когда/почему/как/обсуждали ли что-то \
определённое, вопрос про конкретного человека или тему и т.п.) -- отвечай на него прямо, обычно в 1-4 \
предложениях, только тем, что реально нужно для ответа. НЕ пересказывай весь период, НЕ перечисляй \
темы, которых вопрос не касается, НЕ добавляй жирные заголовки тем -- это прямой ответ, а не сводка. \
Правила про "САМОЕ ИНТЕРЕСНОЕ / ОСТАЛЬНОЕ" ниже в этом случае не применяются вообще.

Если же это общий запрос вида "что происходило"/"перескажи чат", или вопрос настолько широкий, что \
естественным ответом будет обзор основных тем -- тогда разделяй темы по значимости на два уровня, \
