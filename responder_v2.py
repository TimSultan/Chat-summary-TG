...
REPLY_STYLE_RULE = """\
Это отправляется прямо как ответное сообщение в чат Telegram, а не сохраняется как документ, но НЕ \
жертвуй содержанием ради краткости -- лучше несколько содержательных предложений с именами и \
подробностями, чем один общий заголовок ни о чём:
- Никаких заголовков "## ". Для ответа по темам: **жирным** короткое название темы, затем 1-3 \
предложения с конкретикой -- кто что сказал/предложил, какие позиции столкнулись, чем закончилось. \
Не сжимай до одной обтекаемой фразы тему, по которой была реальная развёрнутая дискуссия.
- Освещай все темы, где была реальная содержательная дискуссия, а не только 3-4 самые крупные -- \
лучше упомянуть тему одним ёмким предложением, чем совсем пропустить. Пропускай только то, что и так \
велено пропускать выше (приветствия, малосодержательные обмены и т.п.).
- Длину определяет содержание, а не наоборот: не обрезай важные детали ради короткого ответа, но и \
не растягивай то, что по сути укладывается в одно предложение. Тихий день -- короткий ответ, активный \
день с несколькими насыщенными темами -- длинный, и это нормально.
- Никаких вступлений ("Вот сводка...") и заключительных фраз.
"""
