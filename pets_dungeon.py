...
BOSS_WEAKNESS: Final = {
    ...
    "gatekeeper": "Учит не кнопки, а привычки: что у тебя идёт за чем. Он честно говорит, чего ждёт и насколько уверен. "
                  "Сломай уверенный прогноз — откроется замок; сделай ожидаемое — замок закроется обратно. "
                  "Один и тот же обман дважды — потолок.",
...
SHOP_STOCK: Final = (
    ...
    _stock("phoenix_totem", "🔥", "Тотем Феникса",
           "Один раз воскрешает в текущем забеге. Первый достаётся бесплатно за победу "
           "над Фениксом; здесь продаётся замена, когда он сгорел. Покрась фигурку "
           "Феникса в квестах, чтобы получать тотем в начале каждого забега.",
