...
BOOT_SPECS: Final[tuple[GearSpec, ...]] = (
    ...
    _spec("bt39", "Ботинки с кнопкой в носке", "Каждый удар оставляет маленькое, но обидное напоминание.", "boots", "rare", (("strength", 4), ("luck", 3), ("armor", -1)), _effect("bleed", "Кровотечение: 3% урона за ход за заряд, до конца боя. На полной ране лечение соперника режется на 30%.", 3, heal_cut=30)),
    _spec("bt40", "Берцы с гвоздями в подошве", "Напоминания копятся, и уже совсем не маленькие.", "boots", "legendary", (("strength", 8), ("luck", 6), ("armor", -3)), _effect("bleed", "Кровотечение: 8% урона за ход за заряд, до конца боя. На полной ране лечение соперника режется на 55%.", 8, heal_cut=55)),
)
...
GLOVE_SPECS: Final[tuple[GearSpec, ...]] = (
    ...
    _spec("gl32", "Варежки последнего кассира", "Сдачу не дают, удары возвращают полностью.", "gloves", "legendary", (("armor", 10), ("luck", 5), ("agility", -4)), _effect("perfect_parry", "Первый полученный удар слабее на 80%; поглощённый урон добавляется к следующей атаке.", 80)),
    ...
    _spec("gl36", "Перчатки вратаря сборной двора", "Пенальти теперь назначает даже без повода.", "gloves", "legendary", (("armor", 9), ("strength", 5), ("agility", -3)), _effect("perfect_parry", "Первый полученный удар слабее на 90%; поглощённый урон добавляется к следующей атаке.", 90)),
    ...
    _spec("gl39", "Перчатки с молотым перцем", "Прикосновение жжётся, соперник щурится ещё пару ходов.", "gloves", "rare", (("strength", 4), ("luck", 4), ("health", -1)), _effect("venom_blade", "Отравляет на 5% урона. Следующая атака соперника: 16% промаха и −6% урона.", 16, poison=5, weaken=6)),
    _spec("gl40", "Перчатки с контрабандным перцем чили", "Слёзы соперника теперь не высыхают два раунда подряд.", "gloves", "legendary", (("strength", 8), ("luck", 7), ("health", -3)), _effect("venom_blade", "Отравляет на 9% урона. Следующая атака соперника: 28% промаха и −12% урона.", 28, poison=9, weaken=12)),
)
