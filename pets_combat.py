...
number. That is the reading that actually produces pets_config's promised "6-12 rounds
... a level-1 pair trade ~11 blows, a maxed pair ~8": with one blow per round the numbers
come out roughly double (both sides only land a hit every other round), which is not what
the tuned constants were aimed at. Leadership itself alternates round to round from the
rng-picked first mover, so nobody is stuck permanently swinging second.
"""
...
@dataclass(frozen=True)
class FightResult:
    winner: str           # Fighter.key
    loser: str
    rounds: tuple
    opening: str          # flavour line
    closing: str          # flavour line
    total_damage: dict
    stopped_early: bool   # MAX_ROUNDS hit, awarded on remaining HP share
...
def simulate(a: "Fighter", b: "Fighter", rng=None) -> "FightResult":
    ...
    for round_number in range(1, C.MAX_ROUNDS + 1):
        ...
        # Nobody died in MAX_ROUNDS -- award on remaining HP share rather than hang. With
        # the tuned constants this branch never fires; it exists so a future item can
        # never turn a fight into an infinite loop on a bot thread.
        stopped_early = True
        frac = {
            key: (hp[key] / derived[key]["max_hp"]) if derived[key]["max_hp"] else 0.0
            for key in (a.key, b.key)
        }
        if frac[a.key] > frac[b.key]:
            winner_key, loser_key = a.key, b.key
        elif frac[b.key] > frac[a.key]:
            winner_key, loser_key = b.key, a.key
        else:
            winner_key, loser_key = (a.key, b.key) if rng.random() < 0.5 else (b.key, a.key)

    closing = pets_flavor.line("victory", fighters[winner_key].name, fighters[loser_key].name, rng=rng)
